        ...
        if inp_post_response.status_code != 404:
            ...
            if len(hotel_row) > 0:
                ...
                try:
                    ...
                    return JSONResponse(content=msg, headers=headers, status_code=status.HTTP_503_SERVICE_UNAVAILABLE)
                # --------------------------------------------------------------------
...
def get_me(request: Request):
    ...
    if username_from_header is not None:
        ...
        try:
            ...
            loyalty = None
            loyalty_F = False
            # return JSONResponse(content=msg, headers=headers, status_code=status.HTTP_200_OK)

        if loyalty_F is not None or inp_post_response.status_code != 404:
            if loyalty_F is None:
...
def get_loyalty(request: Request):
    ...
    if username_from_header is not None:
        try:
            ...
            inp_post_response = None
        if inp_post_response is not None and inp_post_response.status_code != 404:
            loyalty_row = inp_post_response.json()
            headers = {'Content-Type': 'application/json'}
            return JSONResponse(content=loyalty_row[0], headers=headers, status_code=status.HTTP_200_OK)
        else:
            headers = {'Content-Type': 'application/json'}
            return JSONResponse(content='Loyalty Service unavailable', headers=headers, status_code=status.HTTP_503_SERVICE_UNAVAILABLE)
    else:
        headers = {'Content-Type': 'application/json'}
        return JSONResponse(content='Loyalty Service unavailable', headers=headers, status_code=status.HTTP_503_SERVICE_UNAVAILABLE)
